...
// Combined slot list for "all grades" view (unique by slot_key)
const allRows = (() => {
  const seen = new Set();
  const out = [];
...
// Sets of all recess keys across grades (for showing Recess)
const allRecessKeys = new Set(
  Object.values(gradeConfig).map((cfg) => cfg.recessKey)
);

export const Scheduling_Card = ({
  gradeLevel = 7,
  search = '',
  subjectId = '',
  sectionId = '',
  ...
  // Per-teacher per-slot room text (e.g., 'Room 201')
  ...
  // Protect against auto-save: snapshot + cancel
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      // Advisers for the chosen grade (as before)
      let adv = supabase
        .from('teachers')
        .select(
          `
      ...
      // Seed weekly grid: stamp Recess on all recess keys present in combined grid
      const blankWeekly = () =>
      ...
      if (teacherIds.length) {
        // 1) Load all schedules for these teachers (include section_id for room lookup)
        ...
        // 2) Load HGP teacher_subject rows and find their schedule slots
        ...
        // 3) Lookup rooms for any schedules with a section_id (schema uses room_label)
        const secIds = [
          ...new Set(
        ...
        if (secIds.length) {
          ...
          if (!secErr && Array.isArray(secRows)) {
            ...
            secRoomMap = new Map(); // graceful fallback: leave rooms blank
          }
        }
        ...
        // 5) Stamp HGP on the teacher’s actual HGP slot (if known)
        (tRows || []).forEach((t) => {
          ...
          if (weekly && hgpSlot && hgpSlot in weekly) {
            ...
            // If the slot is empty, stamp HGP label; otherwise, append marker
            if (!weekly[hgpSlot]) {
  ...
  }, [gradeLevel, sectionId]); // combined view has fixed activeRows; no toggle state
  ...
  const activeTeacher = useMemo(
    () => teachers.find((t) => t.teacher_id === activeTeacherId) || null,
