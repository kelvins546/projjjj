// src/components/admin_comp/Scheduling_Card_Gr10.jsx
...
export const Scheduling_Card_Gr10 = ({
  ...
  const [teachersGr10, setTeachersGr10] = useState([]);
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      // Advisory-only Grade 10 teachers via INNER JOIN on advisory_section
      let adv = supabase
        .from('teachers')
        .select(
          `
        ...
        .eq('advisory_section.grade_level', 10);
      ...
      (teachers || []).forEach((t) => {
        ...
        const advisory =
          Number(t?.advisory_section?.grade_level) === 10 &&
      ...
      setTeachersGr10(teachers || []);
      setSectionsPerTeacher(seeded);
    } catch (e) {
      console.error(e);
      setErrMsg('Failed to load Grade 10 teachers.');
    } finally {
      setLoading(false);
    }
  }, [sectionId]);
  ...
  const activeTeacher = useMemo(
    () => teachersGr10.find((t) => t.teacher_id === activeTeacherId) || null,
    [teachersGr10, activeTeacherId]
  );
  ...
  const advisoryName = (t) =>
    Number(t?.advisory_section?.grade_level) === 10 && t?.advisory_section?.name
      ? t.advisory_section.name
      : '—';

  const filteredTeachers = useMemo(() => {
    const q = (search || '').trim().toLowerCase();
    return (teachersGr10 || []).filter((t) => {
      const matchesDept =
        selectedDept === 'all' ||
  ...
  }, [teachersGr10, search, selectedDept]); // client filter
  ...
  if (!filteredTeachers.length) {
    return (
      <div className="faculty-card">
        <div className="faculty_card_header_grade10"></div>
        <div className="faculty-card-body">
          <p>No Grade 10 teachers found.</p>
        </div>
      </div>
    );
  }
