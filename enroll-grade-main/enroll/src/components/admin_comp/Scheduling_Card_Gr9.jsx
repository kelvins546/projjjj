// src/components/admin_comp/Scheduling_Card_Gr9.jsx
...
export const Scheduling_Card_Gr9 = ({
  search = '',
  subjectId = '', // ignored for advisory-only view
  sectionId = '',
  selectedDept = 'all',
}) => {
  ...
  const [teachersGr9, setTeachersGr9] = useState([]);
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      // Advisory-only Grade 9 teachers via INNER JOIN on advisory_section
      let adv = supabase
        .from('teachers')
        .select(
          `
        ...
        .eq('advisory_section.grade_level', 9);
      ...
      (teachers || []).forEach((t) => {
        ...
        const advisory =
          Number(t?.advisory_section?.grade_level) === 9 &&
      ...
      setTeachersGr9(teachers || []);
      setSectionsPerTeacher(seeded);
    } catch (e) {
      console.error(e);
      setErrMsg('Failed to load Grade 9 teachers.');
    } finally {
      setLoading(false);
    }
  }, [sectionId]);
  ...
  const activeTeacher = useMemo(
    () => teachersGr9.find((t) => t.teacher_id === activeTeacherId) || null,
    [teachersGr9, activeTeacherId]
  );
  ...
  const advisoryName = (t) =>
    Number(t?.advisory_section?.grade_level) === 9 && t?.advisory_section?.name
      ? t.advisory_section.name
      : '—';

  const filteredTeachers = useMemo(() => {
    const q = (search || '').trim().toLowerCase();
    return (teachersGr9 || []).filter((t) => {
      const matchesDept =
        selectedDept === 'all' ||
  ...
  }, [teachersGr9, search, selectedDept]); // client filter [web:154]
  ...
  if (loading) {
    return (
      <div className="faculty-card">
        <div className="faculty_card_header_grade9"></div>
        <div className="faculty-card-body">
          <p>Loading Grade 9 teachers…</p>
        </div>
      </div>
    );
  }
